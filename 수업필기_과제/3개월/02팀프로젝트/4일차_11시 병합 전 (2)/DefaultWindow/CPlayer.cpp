...
void CPlayer::Create_Bullet()
{
	...
	case BUL_ARM:
		...
		{
			auto iter = find_if(CObjMgr::Get_Instance()->Get_List(OBJ_BULLET)->begin(),
								CObjMgr::Get_Instance()->Get_List(OBJ_BULLET)->end(),
								[](CObj* _bullet) { return dynamic_cast<CSuper_Arm*>(_bullet) != nullptr; });


			if (iter != CObjMgr::Get_Instance()->Get_List(OBJ_BULLET)->end())
				break;
			m_bHoldBullet = true;
